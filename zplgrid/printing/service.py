...
def dispatch_document(
    ...
    *,
    ...
    backend = backend_for(configured_printer)
    bytes_sent = 0
    job_ids: list[str] = []
    job_states: list[str] = []
    for page in prepared_pages:
        result = backend.dispatch(driver.prepare(page, configured_printer), configured_printer)
        bytes_sent += result.bytes_sent
        if result.job_id:
            job_ids.append(result.job_id)
        if result.job_state:
            job_states.append(result.job_state)
